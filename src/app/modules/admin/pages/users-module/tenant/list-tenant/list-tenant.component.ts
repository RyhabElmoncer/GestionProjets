  ...
  styles: [
    ...
    `
      .inventory-grid {
        ...
        @screen lg {
          grid-template-columns: 180px 180px 140px 140px 140px 140px 140px 100px 140px 140px ;
        }
      }
    `,
  ],
...
export class ListTenantComponent implements OnInit , OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    if (this.currentUser && this.currentUser.role) {
      ...
      if (role === "ADMIN") {
        ...
        this.isDelete= true;
        this.isUpdate= true;
        // Charge les tenants pour la première fois
        this._adminService.getAdmins$('TENANT', 0, 20);
      }
      else if (role === "SUB_ADMIN") {
        this.isSubadmin = true;
        //todo modifier la liste de TENANTS sous le sub admin et non pas tous les owners
        // Charge les TENANTS pour la première fois
        this._adminService.getAdmins$('TENANT', 0, 20);
        ...
        this.isRead = privilegeSet.has("USERS:TENANTS:READ");
        this.isWrite = privilegeSet.has("USERS:TENANTS:WRITE");
        this.isUpdate = privilegeSet.has("USERS:TENANTS:UPDATE");
        this.isDelete = privilegeSet.has("USERS:TENANTS:DELETE");
      }
      else if (role === "OWNER") {
        this.isOwner = true;
        //todo modifier la liste de TENANTS sous le Owner et non pas tous les owners
        // Charge les TENANTS pour la première fois
        this._adminService.getAdmins$('TENANT', 0, 20);
        // verification des privileges
        const privilegeSet = new Set(this.currentUser.privilegeStrings);
  ...
  onPageChange(event: PageEvent): void {
    this._adminService.getAdmins$('TENANT', event.pageIndex, event.pageSize);
  }
  ...
  navigateToEdit(user: User): void {
    ...
    this.router.navigate(['/pages/tenants/edit-tenant'], { state: { user } });
  }
}
