...
@Component({
  selector: 'update-unit-component',
...
// Méthode  pour obtenir le FormArray des services sélectionnés
  ...
  updateUnit(): void {
    ...
    this.unitService.createUnit(formData)
        .subscribe({
          next: (response) => {
            console.log('unit modifié avec succès');
            this._fuseConfirmationService.open({
              title: this.translate.translate(`Succès`),
              message: this.translate.translate(`unit modifié avec succès`),
              "icon": {
                "show": true,
          ...
          error: (error) => {
            console.error('Error while modifying unit!', error);
            this._fuseConfirmationService.open({
              title: this.translate.translate(`Erreur`),
              message: this.translate.translate(`Error while modifying unit!`),
              "icon": {
                "show": true,
  ...
  patchUnitForm(): void {
          if (this.unit) {
            // patchValue
            this.unitStepperForm.patchValue({
              step1: {
                ...
                bedrooms: this.unit.bedrooms ?? null,
                bathrooms: this.unit.bathrooms ?? null,
                size: this.unit.size ?? null,
                typeOfSize: this.unit.typeOfSize ?? null,
                marketRent: this.unit.marketRent ?? null,
                accessControl: this.unit.accessControl ?? null,
                descriptionFr: this.unit.descriptionFr ?? '',
                descriptionEn: this.unit.descriptionEn ?? '',
                isHome: this.unit.isHome ?? true,
              },
              step2: {
                defaultImage: this.unit.defaultImage || '',
                images: this.unit.images || [],
              }
            });
            const selectedServicesArray = this.unitStepperForm.get('step3.selectedServices') as FormArray;
            if (this.unit.serviceIds?.length) {
              this.unit.serviceIds.forEach(serviceId => {
                selectedServicesArray.push(new FormControl(serviceId));
              });
            }
            // Patch les inclusions avec qty et other
            const selectedInclusionsArray = this.unitStepperForm.get('step4.selectedInclusions') as FormArray;
            // Vider le FormArray existant (important si vous mettez à jour plusieurs fois)
            while (selectedInclusionsArray.length !== 0) {
              selectedInclusionsArray.removeAt(0)
            }

            if (this.unit.unitInclusions?.length) {
              this.unit.unitInclusions.forEach(inclusion => {
                const inclusionGroup = this._formBuilder.group({
                  inclusionId: [inclusion.inclusionId],
                  qty: [inclusion.qty || ''], // Utilisez des valeurs par défaut si nécessaire
                  other: [inclusion.other || '']  // Utilisez des valeurs par défaut si nécessaire
                });
                selectedInclusionsArray.push(inclusionGroup);
              });
            }
          }
        }
